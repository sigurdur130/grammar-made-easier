        ...
        {sortedDifficulties.map((difficulty) => {
          ...
          return (
            <div key={difficulty} className={`space-y-6 p-4 md:p-6 rounded-lg border ${borderClass} ${bgClass}`}>
              <div className="flex items-center backdrop-blur py-2 px-3 rounded-md sticky -top-8 md:-top-6 z-10">
                {icon}
              ...
              <div className="space-y-8">
                {Object.entries(groupedByDifficulty[difficulty]).map(([wordCategory, subs]) => (
                  <div key={`${difficulty}-${wordCategory}`} className="space-y-3 md:space-y-4">
                    <h3 className="text-lg md:text-xl font-semibold text-left pl-2 sticky top-3 md:top-6 backdrop-blur py-2 z-5">
                      {wordCategory}
                    </h3>
