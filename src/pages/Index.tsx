...
const getDifficultyInfo = (difficulty: string | undefined) => {
  switch (difficulty?.toLowerCase()) {
    case "beginner":
      return { 
        order: 1, 
        icon: <Sprout className="h-5 w-5 text-green-500 mr-2" />,
    ...
    case "low intermediate":
      return { 
        order: 2, 
        icon: <Leaf className="h-5 w-5 text-blue-500 mr-2" />,
    ...
    case "high intermediate":
      return { 
        order: 3, 
        icon: <TreePine className="h-5 w-5 text-orange-500 mr-2" />,
    ...
    case "advanced":
      return { 
        order: 4, 
        icon: <Mountain className="h-5 w-5 text-purple-500 mr-2" />,
                    ...
                    <div className="space-y-8">
                      {Object.entries(groupedByDifficulty[difficulty]).map(([wordCategory, subs]) => (
                        <div key={`${difficulty}-${wordCategory}`} className="space-y-3 md:space-y-4">
                          <h3 className="text-lg md:text-xl font-semibold text-left pl-2 sticky top-12 md:top-12 py-2 z-5 backdrop-blur-sm">
                            {wordCategory}
                          </h3>
